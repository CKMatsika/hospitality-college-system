...
@section('content')
    ...
    <div class="mb-8">
        ...
        <p class="text-gray-600 mt-2">Update fee structure details.</p>
    </div>
        ...
        <form action="{{ route('finance.fee-structures.update', $feeStructure) }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="number" id="amount" name="amount" value="{{ old('amount', $feeStructure->amount) }}" 
                ...
                <div>
                    ...
                    <select id="payment_type" name="payment_type" required
                        ...
                        <option value="full" {{ old('payment_type', $feeStructure->payment_type) == 'full' ? 'selected' : '' }}>Full Payment</option>
                        <option value="installment" {{ old('payment_type', $feeStructure->payment_type) == 'installment' ? 'selected' : '' }}>Installment</option>
                        <option value="semester" {{ old('payment_type', $feeStructure->payment_type) == 'semester' ? 'selected' : '' }}>Per Semester</option>
                    </select>
                    @error('payment_type')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea id="description" name="description" rows="3"
                              class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">{{ old('description', $feeStructure->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="mt-6 p-4 bg-gray-50 rounded-lg">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                    <div>
                        <span class="text-gray-500">Fee Structure ID:</span>
                        <span class="text-gray-900 ml-2">#{{ $feeStructure->id }}</span>
                    </div>
                    <div>
                        <span class="text-gray-500">Created:</span>
                        <span class="text-gray-900 ml-2">{{ $feeStructure->created_at->format('M d, Y') }}</span>
                    </div>
                    <div>
                        <span class="text-gray-500">Last Updated:</span>
                        <span class="text-gray-900 ml-2">{{ $feeStructure->updated_at->format('M d, Y') }}</span>
                    </div>
                </div>
            </div>
